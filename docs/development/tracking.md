...
| Hangfire workflow testing harness | Coordinator | Hangfire vNext — HF-2 class-based job discovery | Completed | `HangfireWorkflowTestCatalogBuilderExtensions.RegisterWorkflowTests<TJobBase>`/`RegisterWorkflowTests<TJobBase, TOptions>` invoke a discovered job directly (DI-resolved or `ActivatorUtilities`-activated, including `ILogger<T>` constructor parameters) without a `BackgroundJobServer` or Hangfire storage, so a CLI host can run a single job and return its complete `ILogger` transcript for agent debugging. A caller-supplied timeout is enforced via a linked `CancellationToken`, but cancellation is cooperative only — a non-cooperative job can continue running past the requested timeout. Transcripts can optionally be written to an `ArtifactDirectory` file. 10 new focused tests pass; full `DotNetAgentSurface.Hangfire` suite (59 tests) passes with no regressions. MCP exposure deferred pending confirmation that the `ModelContextProtocol` C# SDK supports the spec's long-running-work concept (see `testing-and-open-decisions.md`). |
...
- [x] Add a Hangfire workflow testing harness (`RegisterWorkflowTests<TJobBase>`/`RegisterWorkflowTests<TJobBase, TOptions>`) that invokes a discovered job directly, captures its `ILogger` transcript, enforces a cooperative timeout, and optionally writes the transcript to an artifact file — letting a CLI host debug a single job without a `BackgroundJobServer` or Hangfire storage.
